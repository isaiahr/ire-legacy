...
run a n = (testmsg a) >> (run (next a) (n+1))
...
testmsg repr = do
    result <- test repr
    if result then putStrLn ("SEGV" ++ show (map prodToken repr)) else putStr ""

-- runs a test with a given representation
test :: [Integer] -> IO Bool
test repr = (writeTmpFile (map (getStr . prodToken) repr) >> runprog)
...
runprog = do
    ...
    (_, Just sout, Just serr, ph) <- createProcess cmd {std_out = CreatePipe, std_err = CreatePipe}
